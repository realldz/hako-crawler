...
export class EpubDeconstructor {
    ...
    async parseToc($opf) {
        ...
        const navItem = Array.from(this.manifest.values()).find((item) => item.mediaType === 'application/xhtml+xml' && item.href.includes('nav'));
        if (navItem) {
     ...
     */
    async updateHtmlContent(content, volSlug, chapterIndex, chapterItemHref) {
        if (!content || !this.zip)
            return '';
     ...
     */
    async processChapterItem(itemHref, tocTitle, volSlug, chapterIndex) {
        if (!this.zip)
            return null;
    ...
    buildVolumeDefinitions() {
        ...
        if (volumes.length === 0) {
            const hrefs = new Set();
            const collectHrefs = (entries) => {
                for (const entry of entries) {
                    if (entry.href)
                        hrefs.add(entry.href);
                    if (entry.children)
                        collectHrefs(entry.children);
                }
            };
     ...
     */
    getOrderedChapterHrefs(volumeHrefs, processedHrefs) {
        const orderedHrefs = [];
    ...
    async deconstruct() {
        ...
        for (const volDef of volumeDefinitions) {
            ...
            const buildTitleMap = (entries) => {
                for (const entry of entries) {
                    if (entry.href)
                        titleMap.set(entry.href, entry.title);
                    if (entry.children)
                        buildTitleMap(entry.children);
                }
            };
...
//# sourceMappingURL=epub-deconstructor.js.map