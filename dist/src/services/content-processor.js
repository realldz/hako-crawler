...
export class ContentProcessor {
     ...
     */
    extractFootnoteDefinitions(html) {
     ...
     */
    convertFootnoteMarkers(html, footnoteMap, chapterSlug) {
        if (!html || typeof html !== 'string') {
     ...
     */
    generateFootnoteAsides(usedNotes, footnoteMap, chapterSlug, includeUnused = true) {
        let footnotesHtml = '';
        const createFootnoteBlock = (noteId, content, title) => {
            const uniqueId = `${chapterSlug}_${noteId}`;
            return `
     ...
     */
    processFootnotes(html, chapterSlug) {
        if (!html || typeof html !== 'string') {
            return '';
        }
        const $ = cheerio.load(html, { xmlMode: false });
        ...
        const { html: markedHtml, usedNotes } = this.convertFootnoteMarkers(processedHtml, footnoteMap, chapterSlug);
        // Step 4: Generate and append footnote asides
        const footnotesHtml = this.generateFootnoteAsides(usedNotes, footnoteMap, chapterSlug, true // Include unused footnotes
        );
     ...
     */
    processContent(html, chapterSlug) {
        if (!html || typeof html !== 'string') {
            return '';
        }
