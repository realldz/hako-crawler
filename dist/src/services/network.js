...
export class NetworkManager {
     ...
     */
    async downloadToFile(url, savePath) {
     ...
     */
    async rotateDomainsAndRetry(originalUrl, headers, timeout) {
        const urlObj = new URL(originalUrl);
        const originalHost = urlObj.hostname.toLowerCase();
        // Determine which domain list to use
        const isImageUrl = this.imageDomains.some((d) => originalHost === d || originalHost.endsWith(`.${d}`));
