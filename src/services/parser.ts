...
import type { LightNovel, Volume, Chapter, ParseResult } from '../types';
...
export class NovelParser {
     ...
     */
    async parse(url: string): Promise<ParseResult<LightNovel>> {
     ...
     */
    private parseHtml($: cheerio.CheerioAPI, url: string): LightNovel {
    ...
    private validateUrl(url: string): string | null {
        ...
        try {
            ...
            const isHakoDomain = DOMAINS.some(
     ...
     */
    private extractMainCover($: cheerio.CheerioAPI): string {
        const coverDiv = $('div.series-cover');
        if (!coverDiv.length) return '';
    ...
    private extractTags($: cheerio.CheerioAPI): string[] {
        ...
        if (!genreDiv.length) return tags;
     ...
     */
    private extractVolumes($: cheerio.CheerioAPI, baseUrl: string): Volume[] {
    ...
    private extractChapters(
        $: cheerio.CheerioAPI,
        volumeElement: Parameters<cheerio.CheerioAPI>[0],
        baseUrl: string
    ): Chapter[] {
        const chapters: Chapter[] = [];

        const ul = $(volumeElement).find('ul.list-chapters');
        if (!ul.length) return chapters;
...
export function deserializeNovel(json: string): LightNovel {
    ...
    return {
        ...
        volumes: parsed.volumes.map((vol: Volume) => ({
            url: vol.url ?? '',
            name: vol.name,
            coverImg: vol.coverImg ?? '',
            chapters: vol.chapters.map((chap: Chapter) => ({
                name: chap.name,
                url: chap.url,
            })),
        })),
