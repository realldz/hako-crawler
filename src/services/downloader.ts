...
export class NovelDownloader {
    ...
    constructor(
        private readonly novel: LightNovel,
    ...
    async createMetadataFile(): Promise<void> {
        ...
        const volumeList: VolumeMetadata[] = this.novel.volumes.map((vol, i) => ({
            order: i + 1,
            name: vol.name,
    ...
    private async processChapter(
        ...
        try {
            ...
            const { html: processedHtml, usedNotes } = this.contentProcessor.convertFootnoteMarkers(
                htmlContent,
                footnoteMap,
                chapterSlug
            );
            ...
            const footnotesHtml = this.contentProcessor.generateFootnoteAsides(
                usedNotes,
                footnoteMap,
                chapterSlug,
                true
    ...
    private async downloadChapterImages(
        $: cheerio.CheerioAPI,
        contentDiv: ReturnType<cheerio.CheerioAPI>,
        imgPrefix: string,
        chapterIndex: number
    ): Promise<void> {
     ...
     */
    private async loadExistingChapters(
        jsonPath: string
    ): Promise<Map<string, ChapterContent>> {
        const existingChapters = new Map<string, ChapterContent>();

        try {
            const oldData = await readJson<VolumeData>(jsonPath);
